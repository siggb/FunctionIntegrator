...
///  a controller for @p WSDatum.
///
///  The data is stored as an @p NSMutableArray of @p NSDatum. The
///  array contains a entry for every separate datum point. A couple
///  of factory methods allows the construction both from Cocoa-style
///  @p NSArray of @p NSNumber and from C-style @p float[] arrays.
///
///  Starting with PowerPlot v1.3 this class supports KVO
///  notifications for the @p values property which are triggered if
///  any element is changed using any of the @p set... methods in the
///  @p WSDatum class or a method in this @p WSData class that
///  modifies the data. Note that the changes to the content are *not*
///  reported when notifications are posted!
///
///  @note The convention that data coordinates are prepended with a
///        capital @p D does not apply here since everything @p WSData
